	...
	*/
	...
	function rfr2b_page_list_recursive($parentid=0,$exclude='',$selected=array(),$show_in_page, $page_name){  
		...
		$total_page = count($pages);
			...
			if( $page_count > 4 ) { 
				if( $page_count == 5 ) {
					$str .='<a style="cursor:pointer; color:#0033CC" onclick="__rfr2b_catpage_openit(\''.$page_name.'_openit\',\''.$page_name.'_closeit\')" id="'.$page_name.'_closeit"><strong>More...</strong></a>';
					$str .='<span id="'.$page_name.'_openit" style="display:none;">';
				} 
				
				$str .= '
			<li><input type="checkbox" name="rfr2b['.$show_in_page.'][]" value="'.$p->ID.'" id="pageid_'.$p->ID.'"'.(($sel)?' checked="checked"':'').' /> <label for="pageid_'.$p->ID.'">'.wp_specialchars($p->post_title).'</label>'.$this->rfr2b_page_list_recursive($p->ID,$exclude,$selected,$show_in_page,'').'</li>';
			
				if( $total_page == $page_count ) {
					$str .='<a style="cursor:pointer; color:#0033CC" onclick="__rfr2b_catpage_closeit(\''.$page_name.'_openit\',\''.$page_name.'_closeit\')" ><strong>Close</strong></a></span>';
				}
